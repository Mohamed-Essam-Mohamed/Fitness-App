...
class ProfileCubit extends Cubit<ProfileState> {
  ProfileCubit(this._getDataProfileUseCase, this._updateDataProfileUseCase,this._updateProfilePhoto)
...
Future<void>  _updatPhoto(File photo)async{
  emit(state.copyWith(updateProfileStatus: Status.loading,errorMessage: null));
    final pref = await SharedPreferences.getInstance();
    final token = pref.getString(AppValues.token);
    final result=await  _updateProfilePhoto(photo,token!);
    switch(result){
     case SuccessResult():
      emit(state.copyWith(updateProfileStatus: Status.success,errorMessage: null));

      case FailureResult():
      emit(state.copyWith(updateProfileStatus: Status.failure,errorMessage: null)); 
    }
